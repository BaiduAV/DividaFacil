...
def create_test_user():
    ...
    # Create test user
    user_data = {"email": "tester@example.com", "password": "test123", "name": "Test User"}
...
def create_complete_test_setup():
    ...
    # Create a new group
    print("\n📁 Creating test group...")
    group_data = {"name": "Category Test Group", "description": "Testing category functionality"}
    ...
    for expense_data in test_expenses:
        ...
        if response.status_code in [200, 201]:
            ...
            print(
                f"❌ Failed to create expense '{expense_data['description']}': {response.status_code} - {response.text}"
            )
