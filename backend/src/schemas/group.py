...
class GroupResponse(BaseModel):
    ...
    def from_group(cls, group: Group) -> "GroupResponse":
        ...
        return cls(
            ...
            expenses=[ExpenseResponse.from_expense(expense) for expense in group.expenses],
            balances=balances,
        )
    ...
    def _calculate_group_specific_balances(cls, group: Group) -> Dict[str, float]:
        ...
        for expense in group.expenses:
            ...
            if expense.split_type == "EQUAL":
                ...
                portions = {
                    uid: round(per_person, 2) for uid in expense.split_among if uid in group.members
                }
                ...
                if abs(diff) > 0.001:
                    ...
                    candidates = [
                        uid
                        for uid in expense.split_among
                        if uid != expense.paid_by and uid in group.members
                    ]
                    if not candidates:
                        candidates = [uid for uid in expense.split_among if uid in group.members]
                    if candidates:
                        portions[candidates[-1]] = round(
                            portions.get(candidates[-1], 0.0) + diff, 2
                        )
