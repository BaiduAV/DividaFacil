...
class NotificationService:
    ...
    def get_upcoming_installments(self, groups: Dict[str, Group], days_ahead: int = 3) -> List[OverdueInstallment]:
                    ...
                    if not installment.paid and today <= due_date <= future_date:
                        ...
                        for user_id in expense.split_among:
                            if user_id != expense.paid_by:  # Don't notify the payer
                                user = group.members.get(user_id)
                                if user:
                                    upcoming.append(OverdueInstallment(
                                        user=user,
                                        expense=expense,
                                        installment=installment,
                                        group=group,
    ...
    def send_overdue_notifications(self, groups: Dict[str, Group]) -> int:
        ...
        sent_count = 0
        for user_id, items in user_overdue.items():
            user = items[0].user
    ...
    def send_upcoming_notifications(self, groups: Dict[str, Group], days_ahead: int = 3) -> int:
        ...
        for item in upcoming_items:
            ...
            if (user.notification_preferences.get('email_upcoming', True) and 
                -item.days_overdue <= user_days_ahead):  # item.days_overdue is negative for upcoming
    ...
    def print_overdue_report(self, groups: Dict[str, Group]) -> None:
        ...
        for user_id, items in user_overdue.items():
            ...
            for item in items:
                ...
                print(f"     Vencimento: {due_date.strftime('%d/%m/%Y')} ({item.days_overdue} dias atraso)")
            print()